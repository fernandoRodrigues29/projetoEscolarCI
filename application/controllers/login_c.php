...
defined('BASEPATH') OR exit('No direct script access allowed'); 
	class Login_c extends CI_Controller {
		...
		function cadastrar() {
			...
			$componentes = array(
				'urlContoroller'=>'login_c/cadastrar',
		...
		function editar($id) {
			...
			// 
			$bdImg = $this->aluno_m->ListarUsuariosComImagemClausula("aluno.id=".$id);
			...
			$componentes = array(
				'urlContoroller'=>'login_c/editar/'.$id,
				'mensagem_retorno'=>NULL,
				'camposHTML'=>$campos,
				'hidden'=>$hidden,
				'avatar'=>$avatar
			...
			if($this->input->post()) {
				...
				$this->form_validation->set_rules($validarCampos);
					...
					$componentes['mensagem_retorno']=array('tipo'=>'alert-danger','msg'=>'erro no cadastro, campos incorretos');							
						...
						$dados['fk_turma'] = $this->input->post('turma');
							...
							if($this->input->post('senha') != ''){
								$dados['senha'] = md5($this->input->post('senha'));	
								$dados = " nome = '".$dados['nome']."', login = '".$dados['login']."' , email =  '".$dados['email']."', senha =  '".$dados['senha']."', fk_turma = ".$dados['fk_turma'];
							}else{
								$dados = " nome = '".$dados['nome']."', login = '".$dados['login']."' , email =  '".$dados['email']."', fk_turma = ".$dados['fk_turma'];
							}
					...
					redirect('/index.php/login_c/gerenciar');	
			} 
		...
		function listar_jsonEncode(){
			$this->load->model('aluno_m');
			$conteudo = $this->aluno_m->ListarUsuariosComImagem();
